...
void salvar()
{
    ...
    else
    {
        ...
        // ABERTURA DE BINARIO PARA SALVAR SOMENTE A QUANTIA DE PRODUTOS
        FILE *pARQESCBIN2 = fopen("produtosBi2.bin", "wb");
        if (pARQESCBIN2 == NULL)
        {
            printf("Erro ao abrir o arquivo produtos.bin");
            exit(1);
        }
        fwrite(&ini, sizeof(int), 1, pARQESCBIN2);
        fclose(pARQESCBIN2);
        // FECHAMENTO DE BINARIO PARA SALVAR SOMENTE A QUANTIA DE PRODUTOS

        free(stock);

        system("cls");
        printf("Os produtos estao agora salvos no arquivo binario produtos 'produtosBi.bin' e a quantidade esta guardada em 'produtosBi2.bin'.\n");
        system("pause");
    }
}